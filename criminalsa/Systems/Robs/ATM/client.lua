addRemoteEvents{"Rob:ATM:Sync:Effects:C"};--addEvents
...
addEventHandler("onClientPreRender",root,function()
    ...
    if(getKeyState("E")and getElementData(localPlayer,"Player:Data:isRobbing")and getElementData(localPlayer,"Player:Data:MarkerRobAllowed"))then
        if(getElementData(localPlayer,"Player:Data:isRobbing")=="ATM")then
            Count=Count+1;
            if(not(Animation))then
                Animation=true;
                triggerServerEvent("Rob:ATM:Sync:Rotation",localPlayer,true);
            end
            if(Count>=Robs["ATM"].Timers.TillOpen)then
                Count=0;
                triggerServerEvent("Rob:Start:ATM",localPlayer);
                if(Animation)then
                    Animation=nil;
                    triggerServerEvent("Rob:ATM:Sync:Rotation",localPlayer);
                end
            end
        end
    else
            ...
            if(v and isElement(v))then
                if(v~=nameSphere and getElementType(v)=="colshape")then
                    if(isElementWithinColShape(localPlayer,v))then
                        if(getElementData(v,"Col:Data:Type")=="ATM")then
                            ...
                            if(getDistanceBetweenPoints3D(x,y,z,vx,vy,vz)<=10)then
                                ...
                                if(x and y)then
                                    if(RobbedAmount<MaxMoney)then
                                        local currentBarCount=146/Robs["ATM"].Timers.TillOpen*Count;
...
addEventHandler("Rob:ATM:Sync:Effects:C",root,function(type,elem)
    if(type and elem)then
        ...
        local effect=createEffect("prt_spark",x1,y1,z1+0.1,-90,0,0);
