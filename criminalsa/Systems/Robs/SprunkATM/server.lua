addRemoteEvents{"Rob:Start:SprunkATM","Rob:SprunkATM:Sync:Rotation","Rob:SprunkATM:Sync:Effects:S"};--addEvents
...
addEventHandler("onResourceStart",resourceRoot,function()
    for atmID,v in pairs(Robs["SprunkATM"].ATMs)do
        ObjectCol[atmID]=createColSphere(v.Pos[1],v.Pos[2],v.Pos[3],v.Col[1]);
        setElementData(ObjectCol[atmID],"Col:Data:Type","SprunkATM");
        GotRobbed[ObjectCol[atmID]]=nil;

        Object[ObjectCol[atmID]]=createObject(v.Object[1],v.Pos[1],v.Pos[2],v.Pos[3],0,0,v.Pos[4]);
        ...
        addEventHandler("onColShapeHit",ObjectCol[atmID],function(hitElem,dim)
            if(hitElem and isElement(hitElem)and getElementType(hitElem)=="player" and isLoggedin(hitElem)and dim)then
                if(not(isPedInVehicle(hitElem)))then
                    ...
                    setElementData(hitElem,"Player:Data:isRobbing","SprunkATM");
                end
            end
        end)
...
addEventHandler("Rob:Start:SprunkATM",root,function()
    if(client and isElement(client)and getElementType(client)=="player" and isLoggedin(client))then
        ...
        if(getPlayerJsonValue(client,"Items",Robs["ATM"].ATMs[RobPlayerMarker2[client]].RequiredItem[1])<Robs["SprunkATM"].ATMs[RobPlayerMarker2[client]].RequiredItem[2])then
            triggerClientEvent(client,"Notify:UI",client,loc(client,"S:Rob:ATM:NeedItem"):format(loc(client,"Item:"..Robs["SprunkATM"].ATMs[RobPlayerMarker2[client]].RequiredItem[1])),"warning",4000);
            return;
        end
        ...
        if(not(getPlayerNotExistingJsonValue(client,"Stats","SprunkATMsRobbed")))then
            setPlayerJsonValue(client,"Stats","SprunkATMsRobbed",1);
        elseif(getPlayerNotExistingJsonValue(client,"Stats","SprunkATMsRobbed"))then
            addPlayerJsonValue(client,"Stats","SprunkATMsRobbed",1);
        end

        removePlayerJsonValue(client,"Items",Robs["SprunkATM"].ATMs[RobPlayerMarker2[client]].RequiredItem[1],Robs["SprunkATM"].ATMs[RobPlayerMarker2[client]].RequiredItem[2]);

        setElementModel(Object[RobPlayerMarker[client]],Robs["SprunkATM"].ATMs[RobPlayerMarker2[client]].Object[2]);
        if(Robs["SprunkATM"].ATMs[RobPlayerMarker2[client]].Object[2]==955)then--new z pos fix
            local x,y,z=getElementPosition(Object[RobPlayerMarker[client]]);
            setElementPosition(Object[RobPlayerMarker[client]],x,y,z-0.6);
        end
        triggerClientEvent(root,"Rob:SprunkATM:Sync:Effects:C",root,true,Object[RobPlayerMarker[client]]);
        ...
        AtmTimer[RobPlayerMarker[client]]=setTimer(function(source,oldID)--reset timer
            ...
            if(oldID==1775)then--old z pos fix
                local x,y,z=getElementPosition(Object[source]);
                setElementPosition(Object[source],x,y,z+0.6);
            end
            GotRobbed[source]=nil;
            
            triggerClientEvent(root,"Rob:SprunkATM:Sync:Effects:C",root,false,Object[source]);--remove client side effects
        ...
        end,Robs["SprunkATM"].Timers.Reset,1,RobPlayerMarker[client],Robs["SprunkATM"].ATMs[RobPlayerMarker2[client]].Object[1])

        --create money pickups
        local x1,y1,z1=getPositionInFrontOfElement(Object[RobPlayerMarker[client]],tonumber(-2.0));
        local Random=math.random(Robs["SprunkATM"].PickupAmount[1],Robs["SprunkATM"].PickupAmount[2]);
        for i=1,Random,1 do
            if(Robs["SprunkATM"].ATMs[RobPlayerMarker2[client]].Object[1]==1900)then
                ...
                if(hitElem and isElement(hitElem)and getElementType(hitElem)=="player" and isLoggedin(hitElem))then
                    if(not(isPedInVehicle(hitElem)))then
                        if(source and isElement(source))then
                            local Reward=math.random(Robs["SprunkATM"].ATMs[RobPickupID[source]].RewardAmount[1],Robs["SprunkATM"].ATMs[RobPickupID[source]].RewardAmount[2]);
                            ...
                            if(tonumber(getElementData(hitElem,"Player:Data:RobbedAmount"))<MaxMoney)then
                                if(tonumber(getElementData(hitElem,"Player:Data:RobbedAmount"))+Reward>=MaxMoney)then
                                    ...
                                    addPlayerJsonValue(hitElem,"Stats","Bounty",math.floor(getElementData(hitElem,"Player:Data:RobbedAmount")/100*Robs["SprunkATM"].BountyPercentage));--give bounty
                                end
...
addEventHandler("Rob:SprunkATM:Sync:Rotation",root,function(type)
    if(client and isElement(client)and getElementType(client)=="player" and isLoggedin(client))then
        if(type)then
...
addEventHandler("Rob:SprunkATM:Sync:Effects:S",root,function()
    if(client and isElement(client)and getElementType(client)=="player")then
        for _,v in ipairs(RobEffects)do
            if(v and isElement(v))then
                triggerClientEvent(client,"Rob:SprunkATM:Sync:Effects:C",client,true,v);
            end
        end
    end
end)