	...
	for i,v in ipairs(CHAT_BUBBLE_TABLE)do
		if(isElement(v.player)and not isPedDead(v.player))then
			if(tick-v.time<CHAT_BUBBLE_TIME)then
				if(getDistanceBetweenPoints3D(pX,pY,pZ,getElementPosition(v.player))<Server.Settings.NametagRange)then
					...
					if(sx and sy)then
						...
						local yPos=interpolateBetween(v.yPos,0,0,sy-50*i,0,0,progress,progress>1 and "Linear" or "OutElastic");
