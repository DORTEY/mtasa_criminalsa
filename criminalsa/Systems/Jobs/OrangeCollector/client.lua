addRemoteEvents{"Job:OrangeCollector:UI"};--addEvent


local Count,JobItem=0,"Orange";

local CEFTimer,BrowserUI,Browser=nil,nil,nil;


addEventHandler("Job:OrangeCollector:UI",root,function(type,amountGot)
    ...
    local PlayerJobLevel=tonumber(getPlayerJsonValue(localPlayer,"Levels","JobOrangeCollectorLVL"))or 1;
    ...
    if(type and type~="nil")then--appear
        ...
        if(not(isTimer(CEFTimer)))then
            ...
            addEventHandler("onClientBrowserCreated",Browser,function()
                ...
                addEventHandler("onClientBrowserDocumentReady",BrowserUI,function(url)
                    if(Count==0)then
                        Count=math.random(Jobs["OrangeCollector"].Drops[PlayerJobLevel][1],Jobs["OrangeCollector"].Drops[PlayerJobLevel][2]);
                    end
        ...
        triggerServerEvent("Job:OrangeCollector:S",localPlayer,tonumber(amountGot));
    end
end)
