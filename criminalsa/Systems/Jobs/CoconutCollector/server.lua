addRemoteEvents{"Job:CoconutCollector:S"};--addEvent
...
local function resetTree(col)
    if(col and isElement(col))then
        setBlipColor(ColBlipMinimap[col],110,80,55,255);
    end
end

addEventHandler("onResourceStart",resourceRoot,function()
    for i,v in pairs(Jobs["CoconutCollector"].Areas)do
        ObjectCol[i]=createColSphere(v.Pos[1],v.Pos[2],v.Pos[3],Jobs.Settings.Ranges[v.ID]);
        setElementData(ObjectCol[i],"Col:Data:Type","CoconutCollector");

        if(v.Blip)then
            ColBlip[ObjectCol[i]]=createBlip(v.Pos[1],v.Pos[2],v.Pos[3],v.Blip[1],20,110,80,55,255,100);
        ...
        ColBlipMinimap[ObjectCol[i]]=createBlip(v.Pos[1],v.Pos[2],v.Pos[3],1,20,110,80,55,255,100);
        ...
        addEventHandler("onColShapeHit",ObjectCol[i],function(hitElem,dim)
            if(hitElem and isElement(hitElem)and getElementType(hitElem)=="player" and isLoggedin(hitElem)and dim)then
                if(not(isPedInVehicle(hitElem))and not(isPedDead(hitElem)))then
                    ...
                    if(not isKeyBound(hitElem,Server.MainInteractionKey,"down",startCollectingCoconut))then
                        bindKey(hitElem,Server.MainInteractionKey,"down",startCollectingCoconut);
                    ...
                    if(not(getPlayerNotExistingJsonValue(hitElem,"Levels","JobCoconutCollectorLVL")))then--set if element data doesnt exist
                        setPlayerJsonValue(hitElem,"Levels","JobCoconutCollectorLVL",1);
                        setPlayerJsonValue(hitElem,"Levels","JobCoconutCollectorEXP",0);
                    end
                end
            end
        end)
        addEventHandler("onColShapeLeave",ObjectCol[i],function(hitElem,dim)
            if(hitElem and isElement(hitElem)and getElementType(hitElem)=="player" and isLoggedin(hitElem)and dim)then
                ...
                if(isKeyBound(hitElem,Server.MainInteractionKey,"down",startCollectingCoconut))then
                    unbindKey(hitElem,Server.MainInteractionKey,"down",startCollectingCoconut);
...
function startCollectingCoconut(player)
    if(player and isElement(player)and getElementType(player)=="player" and isLoggedin(player)and(not client or client==player))then
        ...
        if(not(isTimer(ColTimer[PlayerJobData[player][1]])))then--reset timer
            triggerClientEvent(player,"Job:CoconutCollector:UI",player,true,nil);
...
addEventHandler("Job:CoconutCollector:S",root,function(amount)
    if(client and isElement(client)and getElementType(client)=="player" and isLoggedin(client))then
        ...
        if(not(isTimer(ColTimer[PlayerJobData[client][1]])))then--reset timer
            ...
            ColTimer[PlayerJobData[client][1]]=setTimer(function(col)
                resetTree(col);
            end,Jobs["CoconutCollector"].Timers.Reset,1,PlayerJobData[client][1])

            --give reward
            addPlayerJsonValue(client,"Items","Coconut",tonumber(amount));

            --level
            updateJobLevelstuff(client,"CoconutCollector");
        else
...
local function destroyElementsAfterQuitDead(player)
    ...
    if(player and isElement(player))then
        if(isKeyBound(player,Server.MainInteractionKey,"down",startCollectingCoconut))then
            unbindKey(player,Server.MainInteractionKey,"down",startCollectingCoconut);
