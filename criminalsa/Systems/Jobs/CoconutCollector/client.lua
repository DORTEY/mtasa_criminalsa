addRemoteEvents{"Job:CoconutCollector:UI"};--addEvent


local Count,JobItem=0,"Coconut";

local CEFTimer,BrowserUI,Browser=nil,nil,nil;


addEventHandler("Job:CoconutCollector:UI",root,function(type,amountGot)
    ...
    local PlayerJobLevel=tonumber(getPlayerJsonValue(localPlayer,"Levels","JobCoconutCollectorLVL"))or 1;
    ...
    if(type and type~="nil")then--appear
        ...
        if(not(isTimer(CEFTimer)))then
            ...
            addEventHandler("onClientBrowserCreated",Browser,function()
                ...
                addEventHandler("onClientBrowserDocumentReady",BrowserUI,function(url)
                    if(Count==0)then
                        Count=math.random(Jobs["CoconutCollector"].Drops[PlayerJobLevel][1],Jobs["CoconutCollector"].Drops[PlayerJobLevel][2]);
                    end
        ...
        triggerServerEvent("Job:CoconutCollector:S",localPlayer,tonumber(amountGot));
    end
end)
