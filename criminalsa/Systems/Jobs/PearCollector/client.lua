addRemoteEvents{"Job:PearCollector:UI"};--addEvent


local Count,JobItem=0,"Pear";

local CEFTimer,BrowserUI,Browser=nil,nil,nil;


addEventHandler("Job:PearCollector:UI",root,function(type,amountGot)
    ...
    local PlayerJobLevel=tonumber(getPlayerJsonValue(localPlayer,"Levels","JobPearCollectorLVL"))or 1;
    ...
    if(type and type~="nil")then--appear
        ...
        if(not(isTimer(CEFTimer)))then
            ...
            addEventHandler("onClientBrowserCreated",Browser,function()
                ...
                addEventHandler("onClientBrowserDocumentReady",BrowserUI,function(url)
                    if(Count==0)then
                        Count=math.random(Jobs["PearCollector"].Drops[PlayerJobLevel][1],Jobs["PearCollector"].Drops[PlayerJobLevel][2]);
                    end
        ...
        triggerServerEvent("Job:PearCollector:S",localPlayer,tonumber(amountGot));
    end
end)
